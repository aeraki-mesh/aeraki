...
# If we want to build aeraki in linux amd64 platform,
# just execute make go.build.linux_amd64.aeraki.
.PHONY: go.build.%
go.build.%:
	@$(LOG_TARGET)
...
# Build the aeraki binaries in the hosted platforms.
.PHONY: go.build
go.build: $(addprefix go.build., $(addprefix $(PLATFORM)., $(BINS)))

# Build the aeraki binaries in multi platforms
...
.PHONY: build
build: ## Build aeraki for host platform. See Option PLATFORM and BINS.
build: go.build

.PHONY: build-multiarch
build-multiarch: ## Build aeraki for multiple platforms. See Option PLATFORMS and IMAGES.
