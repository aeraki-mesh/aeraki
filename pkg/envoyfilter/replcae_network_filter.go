...
func GenerateReplaceNetworkFilter(service *networking.ServiceEntry, proxy proto.Message, filterName string, filterType string) *networking.EnvoyFilter {
	var buf []byte
	var err error

	if buf, err = protojson.Marshal(proxy); err != nil {
		//This should not happen
		generatorLog.Errorf("Failed to generate EnvoyFilter: %v", err)
		return nil
	}
